 ...
 */
public class LinearFilter implements PIDSource {
   ...
   */
  public LinearFilter(DoubleSupplier source, double[] ffGains, double[] fbGains) {
    m_source = source;
   ...
   */
  public static LinearFilter singlePoleIIR(DoubleSupplier source,
                                           double timeConstant,
                                           double period) {
    ...
    return new LinearFilter(source, ffGains, fbGains);
  }
   ...
   * @param period       The period in seconds between samples taken by the user
   */
  public static LinearFilter highPass(DoubleSupplier source,
                                      double timeConstant,
                                      double period) {
    double gain = Math.exp(-period / timeConstant);
   ...
   */
  public static LinearFilter movingAverage(DoubleSupplier source, int taps) {
    if (taps <= 0) {
  ...
  public double pidGet() {
    ...
    // Rotate the inputs
    m_inputs.addFirst(m_source.getAsDouble());
