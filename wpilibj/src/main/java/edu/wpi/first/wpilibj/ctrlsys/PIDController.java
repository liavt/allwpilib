...
public class PIDController extends SendableBase implements PIDInterface {
  ...
  private GainNode m_feedforward = new GainNode(0.0, m_refInput);
   ...
   */
  @SuppressWarnings("ParameterName")
  public PIDController(double Kp, double Ki, double Kd, INode feedforward,
                INode input, PIDOutput output, double period) {
    m_sum = new SumNode(m_refInput, true, input, false);
    m_pid = new PIDNode(Kp, Ki, Kd, feedforward, m_sum, period);
    m_output = new Output(m_pid, output, period);
  ...
  public PIDController(double Kp, double Ki, double Kd, INode feedforward,
    ...
    this(Kp, Ki, Kd, feedforward, input, output, INode.DEFAULT_PERIOD);
  }
