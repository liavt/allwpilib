...
class LinearFilter {
 public:
  LinearFilter(SourceNode& input, std::initializer_list<double> ffGains,
               std::initializer_list<double> fbGains);
  LinearFilter(SourceNode& input, std::initializer_list<double> ffGains,
               const std::vector<double>& fbGains);
  LinearFilter(SourceNode& input, const std::vector<double>& ffGains,
               std::initializer_list<double> fbGains);
  LinearFilter(SourceNode& input, const std::vector<double>& ffGains,
               const std::vector<double>& fbGains);

  // Static methods to create commonly used filters
  static LinearFilter SinglePoleIIR(SourceNode& input, double timeConstant,
                                    double period);
  static LinearFilter HighPass(SourceNode& input, double timeConstant,
                               double period);
  static LinearFilter MovingAverage(SourceNode& input, int taps);
...
#include "CtrlSys/LinearFilter.inc"
